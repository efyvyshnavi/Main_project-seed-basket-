...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_request'])) {
  ...
  $query = "INSERT INTO tbl_dbrequest (options, note, db_id,orderid,db_status,sellerid) VALUES ('$delivery_status', '$delivery_notes', '$db_id', '$orderid', '0', '$sellerid')";
...
?>
  ...
  <form method='post' action='#'>
    ...
    <textarea name='delivery_notes' placeholder='Enter delivery notes'></textarea>
    <select name='orderid'>
  <option value=''>----------------------------------Select Order ID------------------------------------</option>
  <?php
$query = "SELECT DISTINCT sa.sellerid, sa.orderid 
          FROM tbl_dbassign sa 
          WHERE sa.db_id='$db_id'";
$result = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($result)) {
  $value = $row['orderid'] . ',' . $row['sellerid'];
  echo '<option value="' . $value . '" style="color: blue;font-weight:bold">' . $row['orderid'] . '</option>';
}
?>
</select>
<input type="hidden" name="seller" id="sellerid" value="">
    <input type='submit' name='update_request' value='Update Request'>
    <br>
  </form>
  <br><br>
</div>
